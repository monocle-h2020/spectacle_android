...
class Global {
    public static ParseFile jpgfile;
    public static ParseFile rawfile;
...
public class Camera2RawFragment extends Fragment
    ...
    private  float fltHeading;
    private long maxFrameDur;
    public double pitch;
    public double raw_pitch;

    public  double degreesfromzenith;
...
//    private static final int MAX_PREVIEW_HEIGHT = 1080;
   ...
   private static final int MAX_PREVIEW_HEIGHT = 5328;
    /**
     * Tag for the {@link Log}.
     */
    private static final String TAG = "Spectacle";
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        tvTilt = (TextView) view.findViewById(R.id.tvTilt);
        tvSunpos = (TextView) view.findViewById(R.id.tvSunpos);
        tvHorizontal = (TextView) view.findViewById(R.id.tvHoriz);
        ...
        // Add listener to button sky/water

        toggleBtnSky.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
        ...
        // Add listener to exposureslider (seekBar)
...
//                Activity activity = getActivity();
                ...
                closeCamera();
                openCamera();
                            //  showToast("Seek bar progress is :" + progressChangedValue);
            }
        });

        // device sensor manager
        // Text views
        location = new SimpleLocation(getActivity().getApplicationContext(), false, false, 5 * 1000, false);
        // Get GPS info
        ...
        location.setListener(new SimpleLocation.Listener() {
            ...
            public void onPositionChanged() {
                dblLat=    location.getLatitude();
                dblLon=    location.getLongitude();
                String    strLat=String.format("%.2f", dblLat);

                String    strLong=String.format("%.2f", dblLon);
                tvLocation.setText("Lat: " + strLat + " Lon: " + strLong);
...
// Calculate sun position
                ...
                AzimuthZenithAngle position = SPA.calculateSolarPosition(
                        dateTime,
                        dblLat, // latitude (degrees)
                        dblLon, // longitude (degrees)
                        190, // elevation (m)
                        DeltaT.estimate(dateTime), // delta T (s)
                        1010, // avg. air pressure (hPa)
                        11); // avg. air temperature (°C)

                    strZA=String.format("%.0f", position.getZenithAngle());
                    strAZ=String.format("%.0f",position.getAzimuth());

                if (dblLat>0 &&  dblLon>0) {
                    tvSunpos.setText("Sun: Alt: " + strZA + "° Az: " + strAZ + "°");
                  //  tvSunpos.setText(strAZ);


                } else
                    tvSunpos.setText("No GPS");
                // store it
            }
     ...
     //   senAccelerometer = mSensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
        senAccelerometer = mSensorManager.getDefaultSensor(Sensor.TYPE_ROTATION_VECTOR);


        mSensorManager.registerListener(this, senAccelerometer , mSensorManager.SENSOR_DELAY_NORMAL);
        ...
        mOrientationListener = new OrientationEventListener(getActivity(),
            ...
            public void onOrientationChanged(int orientation) {
                // Do we need this for iSPEX? Dont think so.
    ...
    private boolean setUpCameraOutputs() {
        ...
        if (manager == null) {
            ErrorDialog.buildErrorDialog("This device doesn't support capturing RAW.").
        ...
        try {
            ...
            for (String cameraId : manager.getCameraIdList()) {
                ...
                // NS experiment, does not work somehow wwith Galaxy S6

                largestJpeg=largestRaw;
    ...
    private void setup3AControlsLocked(CaptureRequest.Builder builder) {
        ...
        Long minrange = range.getLower();
        Long maxrange = range.getUpper();
        // testing switch
    // debug

        maxFrameDur = mCharacteristics.get(CameraCharacteristics.SENSOR_INFO_MAX_FRAME_DURATION);
// invert
        long diff = 197- exposureSlider.getProgress();

        maxFrameDur= maxFrameDur/diff;

        if (maxFrameDur<0) {
            maxFrameDur=maxrange;
        }

  showToast(("Slider: " + exposureSlider.getProgress() + "Current exp: " + maxFrameDur));
        ...
        if (!mNoAFRun) {
            ...
            if (contains(mCharacteristics.get(
                ...
                builder.set(CaptureRequest.CONTROL_MODE, CaptureRequest.CONTROL_AWB_MODE_AUTO);
                builder.set(CaptureRequest.CONTROL_AF_MODE, CaptureRequest.CONTROL_AF_MODE_OFF);
// set infinite

                // Welke waarde is dit? infinite of max
...
// DEBUG 19 mar 2020
             // make this proportional to slider!
    ...
    private void takePicture() {
        synchronized (mCameraStateLock) {
            ...
            try {
                ...
                // Now submit to parse
                ParseObject parse_spectacledata = new ParseObject("android_spectacledata");

                parse_spectacledata.put("manufacturer", Build.MANUFACTURER);
                parse_spectacledata.put("model", Build.MODEL);
                parse_spectacledata.put("sdk", Build.VERSION.SDK_INT);

                parse_spectacledata.put("exposure", maxFrameDur);

                String watersky;

                if (toggleBtnSky.isChecked()) {
                            watersky="sky";
                        } else {
                            watersky="water";
                        }
                ...
                parse_spectacledata.put("water_sky_selection", watersky);


                String ispexon;

                if (toggleBtnhasiSPEX.isChecked()) {
                    ispexon="true";
                } else {
                    ispexon="false";
                }
                ...
                parse_spectacledata.put("ispex_on", ispexon);


                // date, time, GPS coordinates, camera azimuth/zenith angles
                parse_spectacledata.put("lat", location.getLatitude());
                parse_spectacledata.put("lon",  location.getLongitude());



                parse_spectacledata.put("heading",  fltHeading);


                parse_spectacledata.put("raw_pitch",  raw_pitch);
                parse_spectacledata.put("corrected_pitch",  pitch);


                parse_spectacledata.put("sun_alt",  strZA);
                parse_spectacledata.put("sun_az",  strAZ);
            ...
            } catch (CameraAccessException | ParseException e) {
                e.printStackTrace();
            }
        }
    }
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
     ...
     //   double dblDegree=(double)degree;
        ...
        if (fltHeading>135)         tvHorizontal.setText(R.string.arrowleft);
        else
...
/*
            >     Sky, iSPEX OFF: Zenith 40 degrees
                    >     Sky, iSPEX ON: Zenith 57.3 degrees
                    >     Water, iSPEX OFF: Zenith 140 degrees
                    >     Water, iSPEX ON: Zenith 157.3 degrees

*/

             degreesfromzenith=140;
             raw_pitch=pitch; // uncorrected pitch (parse)
            // check if sky is ON,  then use other pitch value.
            if (toggleBtnSky.isChecked()) degreesfromzenith=40;
            ...
            pitch=(pitch-180)*-1; // ---> Out this is horizon not Zenit.

            if (pitch<degreesfromzenith)            tvVert.setText(R.string.arrowdown);

            else
                tvVert.setText(R.string.arrowup);
            // correct for ispex unit


            String    strPitch=String.format("%.2f", pitch);
            tvTilt.setText("Zenith Angle: " + strPitch );
    ...
    private static class ImageSaver implements Runnable {
      ...
      @Override
        ...
        public  void run() {
          ...
          final ParseFile jpgfile ;
          final  ParseFile rawfile;
          ...
          int format = mImage.getFormat();
            switch (format) {
               ...
               try {
                       ...
                       jpgfile = new ParseFile("thumb.jpg", bytes);

                        jpgfile.saveInBackground(new SaveCallback() {
                            public void done(ParseException e) {
                                // Handle success or failure here ...
                                // now pass it
                                Global.jpgfile=jpgfile;
                                ParseObject parse_spectacledata = new ParseObject("raw_data");


                              //  parse_spectacledata.put("thumb", jpgfile);
                              //  parse_spectacledata.saveInBackground();
                ...
                case ImageFormat.RAW_SENSOR: {
                    ...
                    try {
                        ...
                        File    rawInputFile=mFile;
                        int size = (int) rawInputFile.length();
                        byte[] rawBytes = new byte[size];

                        try {
                            BufferedInputStream buf = new BufferedInputStream(new FileInputStream(rawInputFile));
                         ...
                         rawfile = new ParseFile("rawfile.dng", rawBytes);
                        Global.rawfile=rawfile;

                        rawfile.saveInBackground(new SaveCallback() {

                            public void done(ParseException e) {
                                // Handle success or failure here ...

                                ParseObject parse_spectacledata = new ParseObject("raw_data");
                                parse_spectacledata.put("raw", rawfile);
                                parse_spectacledata.put("thumb",Global.jpgfile);
...
// norbert
                            ...
                            public void done(Integer percentDone) {
                                // OK this took some time to figure out
                              Camera2RawFragment A = new Camera2RawFragment();
                              A.showToast(percentDone.toString());
